...
It also creates regional SQS queues, three private immutable ECR repositories,
a private-by-default security group with optional dashboard HTTPS ingress, a
stable Elastic IP, and the instance's least-privilege IAM role.
...
Systemd restarts only start digest-pinned images already selected in the
root-only release file. They never fetch or rebuild source. Build the analysis,
dashboard, and dashboard-admin images off-host with
`scripts/publish-aws-images.sh`, then deploy the three returned digest references
with `sudo deploy-static-log-analysis-images`. The helper verifies every image,
runs migrations, performs a bounded health gate, and restores the previous
digest selection when a candidate fails.
...
The `t3.small` is a runtime host only. Application compilation happens on an
operator workstation or CI runner and cannot consume the instance's journal or
dashboard resources.
...
- `release_image_repositories`: private repositories used by the release
  publisher; the instance role can pull only from these repositories.
...
NAT gateway, load balancer, or orchestration cluster.
